...
export default function StaffProfile() {
  ...
  const { register, handleSubmit } = useForm({
    defaultValues: { name: user?.name ?? '', email: user?.email ?? '', phone: '+91 98765 43211' },
  })
  ...
  return (
    <div className="max-w-xl space-y-6 pb-6">
      ...
      <motion.div className="card p-6 flex items-center gap-5" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="w-16 h-16 rounded-full bg-info/15 flex items-center justify-center flex-shrink-0">
          <span className="font-mono text-2xl font-bold text-info">{user?.name?.[0]?.toUpperCase() ?? 'S'}</span>
        </div>
        <div>
          <h3 className="font-display font-semibold text-lg text-ink">{user?.name ?? 'Staff'}</h3>
          <p className="font-body text-sm text-body/70">{user?.email}</p>
          <span className="inline-block mt-1.5 px-2.5 py-0.5 rounded-full bg-info/10 text-info text-xs font-mono font-semibold capitalize">{user?.role}</span>
          <p className="font-body text-2xs text-body/50 mt-1">Alibag PHC · Raigad</p>
        </div>
      </motion.div>
      ...
      <motion.div className="card p-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        ...
        <form onSubmit={handleSubmit(onSave)} className="space-y-4">
          <div>
            <label htmlFor="sf-name" className="block font-body text-xs font-medium text-ink mb-1.5">{t('auth.fullName')}</label>
            <input id="sf-name" type="text" className="form-input" {...register('name')} />
          </div>
          <div>
            <label htmlFor="sf-email" className="block font-body text-xs font-medium text-ink mb-1.5">{t('auth.email')}</label>
            <input id="sf-email" type="email" className="form-input" {...register('email')} />
          </div>
          <div>
            <label htmlFor="sf-phone" className="block font-body text-xs font-medium text-ink mb-1.5">{t('auth.phone')}</label>
            <input id="sf-phone" type="tel" className="form-input" {...register('phone')} />
          </div>
